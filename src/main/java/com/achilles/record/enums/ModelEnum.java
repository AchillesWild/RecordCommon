...
import com.achilles.record.enums.user.UserHomePageEnum;
...
public enum ModelEnum {
    ...
    //--------------------------------------------- mission  start ----------------------------------------------------------------
    MISSION_ADD("com.achilles.server.controller.mission.MissionCommandController.add", UserHomePageEnum.MISSION.getKey(),1, UserHomePageEnum.MISSION.getValue() + "_新增"),
...
//    MISSION_GET_IMG("com.achilles.server.controller.mission.MissionQryController.getBigImg",512, "待办_获取大图"),
    ...
    ModelEnum(String method, Integer modelCode, Integer functionCode, String desc) {
        this.method = method;
    ...
    static {
        for (ModelEnum modelEnum : EnumSet.allOf(ModelEnum.class)) {
            methodToModelCodeMap.put(modelEnum.getMethod(), modelEnum.getModelCode());
            methodToFunctionCodeMap.put(modelEnum.getMethod(), modelEnum.getFunctionCode());
            functionToDescMap.put(modelEnum.getModelCode() + "_" +modelEnum.getFunctionCode(), modelEnum.getDesc());
        }
    }
